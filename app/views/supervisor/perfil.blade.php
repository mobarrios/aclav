							...
							@if(Auth::user()->supervisor_id != "")
								...
								@if( $designa->supervisor_1_id == Auth::user()->supervisor_id || $designa->supervisor_2_id == Auth::user()->supervisor_id)
									...
									@if($designa->estado == "a Confirmar")
										<td>	
											<a href="perfil_supervisor/confirmar/{{$model->id}}" class="btn btn-xs btn-success confirmaArbitro">Confirmar</a>		
										</td>	

										<td>		
											<form id="frmRechaza{{$model->id}}" action="perfil_supervisor/rechazar" method="post">
