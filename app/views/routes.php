...
Route::group(array('prefix' => 'web'), function()
{
  ...
  Route::get('/masculinonuevo/{id}', function($id)
  {
   ...
   $torneos_ex 			= Torneos::where('temporada_id',$temporada_actual)->get();
   $ex_jugadores        = array();
   $ex_staffs	        = array();
...
*/
  		 	...
  		 	foreach($o2->BuenaFeStaff as $bfs)
  		 	{
  		 	 		...
  		 	 		if(strtotime($bfs->fecha_hasta) >= strtotime(date('d-m-Y')) || $bfs->fecha_hasta != 'actual' && strtotime($bfs->fecha_hasta) != strtotime('31-12-1969') && strtotime($bfs->fecha_hasta) != strtotime('00-00-0000') )
	  		 		{
